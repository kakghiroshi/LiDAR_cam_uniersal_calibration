...
int main(int argc, char** argv) {
    ...
    FileStorage f_calib;
    f_calib.open("../config/calib_fisheye.yaml",
                 FileStorage::READ);
    if (!f_calib.isOpened()) {
    ...
    for (int i = 0; i < nums; i++) {
        ...
        image_dst = cv::imread("../data/img_fisheye/" +
        ...
        if (find_corners_result) {
            cout << i + 1 << endl;
            valid_queue.push_back(std::to_string(i + 1));
    ...
    for (int i = 0; i < rotation_vectors.size(); i++) {
        cv::Mat img_dst;
        img_dst = cv::imread("../data/img_fisheye/" +
                             valid_queue[i] + ".png");
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(
                "../data/pcd_fisheye/" + std::to_string(i + 1) + ".pcd",
